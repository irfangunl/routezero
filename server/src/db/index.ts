...
const DB_PATH = path.resolve(__dirname, "../../data/routezero.db");
...
export function initDb(dbPath?: string): Database.Database {
  ...
  createTables(db);
  ensureModelRankColumns(db);
  ensureCapabilitiesColumn(db);
...
function ensureModelRankColumns(db: Database.Database) {
  const columns = db.prepare("PRAGMA table_info(models)").all() as {
    name: string;
  }[];
  if (!columns.some((col) => col.name === "coding_rank")) {
    db.prepare("ALTER TABLE models ADD COLUMN coding_rank INTEGER").run();
  }
  if (!columns.some((col) => col.name === "research_rank")) {
    db.prepare("ALTER TABLE models ADD COLUMN research_rank INTEGER").run();
  }
}

function ensureCapabilitiesColumn(db: Database.Database) {
  const columns = db.prepare("PRAGMA table_info(models)").all() as {
    name: string;
  }[];
  if (!columns.some((col) => col.name === "capabilities")) {
    db.prepare(
...
/**
 * V15 (May 2026): OpenCode Zen — anonymous free-tier models from
 * opencode.ai/zen. No API key needed; placeholder key enables routing.
 */
...
/**
 * V16 (May 2026): populate coding_rank + research_rank for all models.
 * Defaults to intelligence_rank, then curated overrides.
 */
function migrateModelsV16(db: Database.Database) {
  db.prepare(
    "UPDATE models SET coding_rank = intelligence_rank WHERE coding_rank IS NULL",
  ).run();
  db.prepare(
    "UPDATE models SET research_rank = intelligence_rank WHERE research_rank IS NULL",
  ).run();

  const update = db.prepare(
    "UPDATE models SET coding_rank = ?, research_rank = ? WHERE platform = ? AND model_id = ?",
  );

  const curated: [number, number, string, string][] = [
    [1, 7, "openrouter", "qwen/qwen3-coder:free"],
    [1, 5, "cerebras", "qwen-3-235b-a22b-instruct-2507"],
    [1, 5, "cerebras", "qwen-3-coder-480b"],
    [2, 10, "opencode", "deepseek-v4-flash-free"],
    [3, 11, "mistral", "codestral-latest"],
    [3, 10, "openrouter", "qwen/qwen3-next-80b-a3b-instruct:free"],
    [3, 9, "ollama", "qwen3-coder:480b"],
    [3, 9, "ollama", "qwen3-coder-next"],
    [7, 1, "openrouter", "minimax/minimax-m2.5:free"],
    [6, 2, "sambanova", "DeepSeek-V3.2"],
    [4, 3, "google", "gemini-2.5-pro"],
    [10, 6, "cohere", "command-r-plus-08-2024"],
    [5, 4, "openrouter", "moonshotai/kimi-k2:free"],
    [4, 6, "google", "gemini-2.5-flash"],
    [6, 5, "mistral", "mistral-large-latest"],
    [12, 11, "google", "gemini-2.5-flash-lite"],
  ];

  for (const [cr, rr, platform, modelId] of curated) {
    update.run(cr, rr, platform, modelId);
  }
}

/**
 * V17 (May 2026): OpenCode Go subscription models.
 * Disabled by default — users with Go subscription enable in UI.
 */
...
/**
 * V18 (May 2026): capabilities column + vision/image-gen/embedding models.
 * Sets capabilities on existing vision-capable models. Adds new vision,
 * image-generation, and embedding models to the catalog.
 */
function migrateModelsV18(db: Database.Database) {
  // Set vision capability on existing Google Gemini models
  db.prepare(
    `
    UPDATE models SET capabilities = 'vision'
     WHERE platform = 'google'
       AND model_id IN ('gemini-2.5-pro', 'gemini-2.5-flash', 'gemini-2.5-flash-lite')
  `,
  ).run();
  db.prepare(
    `
    UPDATE models SET capabilities = 'vision'
     WHERE platform = 'google'
       AND model_id IN ('gemini-3.5-flash', 'gemini-3.1-pro-preview', 'gemini-3-flash-preview', 'gemini-3.1-flash-lite-preview')
  `,
  ).run();
  ...
  db.prepare(
    ...
    UPDATE models SET capabilities = 'vision'
     WHERE platform = 'openrouter'
       AND model_id IN ('qwen/qwen3-coder:free', 'qwen/qwen3-next-80b-a3b-instruct:free')
  `,
  ).run();

  const insert = db.prepare(`
    INSERT OR IGNORE INTO models (platform, model_id, display_name, intelligence_rank, speed_rank, size_label, rpm_limit, rpd_limit, tpm_limit, tpd_limit, monthly_token_budget, context_window, capabilities)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  > = [
    // OpenRouter free-pool vision models
    [
      "openrouter",
      "qwen/qwen3-vl:free",
      "Qwen3 VL (free)",
    ...
    [
      "openrouter",
      "stabilityai/stable-image-core:free",
...
function ensureUnifiedKey(db: Database.Database) {
  ...
  if (!existing) {
    const key = `routezero-${crypto.randomBytes(24).toString("hex")}`;
    db.prepare(
