...
const SORT_PRESETS: Record<string, string> = {
  ...
  coding: `${rankCoalesceExpr("m.coding_rank", "coding")} ASC`,
  researching: `${rankCoalesceExpr("m.research_rank", "research")} ASC`,
};
...
fallbackRouter.post("/sort/:preset", (req: Request, res: Response) => {
  ...
  if (!orderBy) {
    res
      ...
      .json({
        error: {
          message: `Unknown preset: ${preset}. Use: intelligence, speed, budget, coding, researching`,
        },
      });
    return;
  }
