...
import { trackRateSpike } from "../services/webhook.js";
...
// on every request, but RouteZero's whole point is to pick the model itself.
// Requesting this id means "let the router decide" — identical to omitting
// `model` entirely.
//
// Extended auto modes: "auto:coding" and "auto:researching" sort the fallback
// chain by benchmark rank at request time instead of the user's drag order.
const AUTO_MODEL_ID = "auto";

type AutoMode = "coding" | "researching";

function parseAutoModel(modelId: string | undefined): {
  isAuto: boolean;
  autoMode?: AutoMode;
} {
  if (!modelId || modelId === AUTO_MODEL_ID) return { isAuto: true };
  if (modelId.startsWith("auto:")) {
    const mode = modelId.split(":")[1] as AutoMode;
    if (mode === "coding" || mode === "researching")
      return { isAuto: true, autoMode: mode };
  }
  return { isAuto: false };
}
  ...
  res.json({
    ...
    data: [
      ...
      },
      {
        ...
        created: 0,
        owned_by: "routezero",
        name: "Auto (Coding) — sorted by coding benchmark rank",
        context_window: null,
      },
      {
        id: "auto:researching",
        ...
        created: 0,
        owned_by: "routezero",
        name: "Auto (Researching) — sorted by research/reasoning rank",
        context_window: null,
      },
      ...models.map((m) => ({
        id: m.model_id,
...
proxyRouter.post("/chat/completions", async (req: Request, res: Response) => {
  ...
  // Explicit `model` field pins routing or selects auto mode. If the catalog
  // has no enabled row matching the requested id, return 400 — silently
  // auto-routing to a different model would be surprising to clients.
  // Sticky-session is the fallback when no `model` field was sent at all.
  let preferredModel: number | undefined;
  let autoMode: AutoMode | undefined;
  const modelChoice = parseAutoModel(requestedModel);
  if (modelChoice.isAuto) {
    autoMode = modelChoice.autoMode;
  ...
  // Detect image content → require vision-capable model
  const requiredCapability = hasImageContent(messages) ? "vision" : undefined;
    ...
    try {
      ...
      if (isRetryableError(err)) {
        ...
        if (
          ...
          trackRateSpike(route.platform, route.modelId, route.keyId);
        lastError = err;
        console.log(
...
proxyRouter.post("/images/generations", async (req: Request, res: Response) => {
  ...
  const { model, prompt, n, size, response_format } = parsed.data;

  try {
    const route = routeForCapability("image_gen", model ?? "auto");
    const result = await route.provider.generateImage(route.apiKey, prompt, {
      model: route.modelId,
      n,
      size,
      response_format,
...
proxyRouter.post("/embeddings", async (req: Request, res: Response) => {
  ...
  try {
    const route = routeForCapability("embeddings", model ?? "auto");
    const result = await route.provider.createEmbeddings(
      route.apiKey,
      input,
      route.modelId,
    );
